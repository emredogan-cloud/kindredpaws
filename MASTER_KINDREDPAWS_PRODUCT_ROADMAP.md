...
- **Objectives (shipped a, c, d — b deferred):** (a) camera intimacy: gentle scale/ease push-in on the pet during care beats, settle-back on completion (reduced-motion holds still); (c) first-visit verb hints: one-time soft pulse on each room's primary prop (device-local seen-set in prefs, not the pet save; master-motion-gated so tests settle); (d) rhythm-aware notifications: on-device preferred-hour band learned from session-open hours (24-bucket histogram in prefs, never leaves the device), applied inside the existing 1–2/day cap and guilt-free templates. **(b) ambient audio pads: consciously DEFERRED** — synthesized room-pad WAVs can't be verified headless (no on-device audio in CI), so shipping looping playback I can't hear would be unverifiable. The `AudioSink` seam + sound toggle already exist; pads drop in later like the Rive rig. Recorded as a deferred sub-item (§7), not claimed as done.
